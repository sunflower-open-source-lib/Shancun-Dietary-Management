...
//        mZBarView.startCamera(Camera.CameraInfo.CAMERA_FACING_FRONT); // 打开前置摄像头开始预览，但是并未开始识别
    ...
    public void onScanQRCodeSuccess(String result) {
        ...
        if (TextUtils.isEmpty ( result )) {
            mZBarView.startSpot ();
            Toast.makeText ( TestScanActivity.this, "扫描失败", Toast.LENGTH_SHORT ).show ();
        } else {
            Toast.makeText ( TestScanActivity.this, "扫描成功", Toast.LENGTH_SHORT ).show ();
            Intent intent = new Intent ();
            switch (order) {
                case "family":
                    intent.putExtra ( "fid", result );
                    setResult ( RESULT_OK, intent );
                    finish ();
                    break;

                case "box":
                    intent.putExtra ( "macip", result );
                    setResult ( RESULT_OK, intent );
                    finish ();
                    break;
            }
